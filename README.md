...
Use GCC to build the RISC-V binaries. Use -O2 or -O3 and use the regular standard extensions: `-march=rv32gc -mabi=ilp32d`. Enable the RISCV_EXPERIMENTAL option for the best performance unless you are using libriscv as a sandbox. Try enabling the instruction decoder cache and see if it's faster for your case. Always enable the page cache. Always enable LTO. Fair warning: It's a bit harder to use Clang for freestanding RISC-V.
