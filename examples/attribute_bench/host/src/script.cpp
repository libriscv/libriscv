...
void Script::machine_setup()
{
	...
	// The guest's malloc/free are these syscalls, so the arena below is the one
	// and only heap -- which is what lets the host hand the guest owned memory
	machine().setup_native_heap(490, m_heap_area, MAX_HEAP);
	machine().setup_native_memory(495);
}

/// @brief The two host functions the guest calls with an attribute tree, one per
/// marshalling strategy. Both build the same host-side tree and return a
/// checksum of it, which is what proves the two paths deliver the same data.
void Script::install_host_functions()
{
	...
	machine_t::install_syscall_handler(SYSCALL_PRINT,
		[] (machine_t& machine) {
			const auto [text] = machine.sysargs<std::string>();
			printf("%s", text.c_str());
