#pragma once
/**
 * A minimal embedding of libriscv: one VM, a native heap that the guest's own
 * malloc uses, and a handful of syscall-based host functions.
 *
 * The native heap is what makes the zero-copy path possible at all: the arena
 * the host allocates from is the same arena the guest frees to, so a tree the
 * host builds in guest memory can be owned -- and freed -- by the guest.
 */
...
struct Script
{
	...
	// Host functions, as syscall numbers. The native heap owns 490..503.
	...
	/// vmcall, so instruction_counter() afterwards is the cost of this call.
