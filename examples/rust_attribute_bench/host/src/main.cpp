/**
 * Zero-copy Rust attributes vs. flat node-array marshalling.
 *
...
int main(int argc, char** argv)
{
	...
	// The layout the host mirror reads through is the layout the guest requested
	// with #[repr(C, u64)]. Ask the guest what it actually got.
	const std::string layout_report = probe_guest_layout(script);
	if (!layout_report.empty()) {
		printf("FAIL: the guest's layout does not match the host mirror:\n%s",
			layout_report.c_str());
		return 1;
	}

	// Whatever the Rust runtime holds on to after boot is not ours, and is
	// subtracted from the leak check at the end
	...
	for (int index = 0; index < 3; index++) {
		...
		const Result in_flat = measure_in(script, [&] {
			...
			// The guest consumes the array, releasing every allocation in it
			return script.call(in_flat_addr, addr, uint64_t(count));
		});
		const Result in_zc = measure_in(script, [&] {
			// The guest takes the tree with one Box::from_raw and drops it, so
			// there is nothing for the host to tear down afterwards
			return script.call(in_obj_addr, createGuestAttributes(script, host_tree));
		});
